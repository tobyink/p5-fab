{
    ...
    sub new {
        ...
        croak "Missing key in constructor: error"
        ...
        my @unknown = grep not(/\A(?:error|message|original_exception|step)\z/),
          keys %{$args};
        @unknown
          and croak(
    ...
    if ($__XS) {
        ...
        *error = sub {
            @_ == 1 or croak('Reader "error" usage: $self->error()');
            $_[0]{"error"};
        };
    }
...
'Predicate "has_original_exception" usage: $self->has_original_exception()'
    ...
    if ($__XS) {
        ...
        *step = sub {
            @_ == 1 or croak('Reader "step" usage: $self->step()');
            $_[0]{"step"};
        };
    }
    ...
    BEGIN {
        ...
        our %DOES =
          ( "Fab::Exception::StepFailed" => 1, "Fab::Exception" => 1 );
    }
