{
    ...
    sub new {
        ...
        blessed( $args->{"blueprint"} )
          && $args->{"blueprint"}->isa("Fab::Blueprint")
          or croak "Type check failed in constructor: %s should be %s",
          "blueprint", "InstanceOf[\"Fab::Blueprint\"]";
        $self->{"blueprint"} = $args->{"blueprint"};
        require Scalar::Util && Scalar::Util::weaken( $self->{"blueprint"} )
          if ref $self->{"blueprint"};
        ...
        my @unknown = grep not(/\A(?:blueprint|definition_context|name)\z/),
          keys %{$args};
        @unknown
          and croak(
    ...
    sub id {
        @_ == 1 or croak('Reader "id" usage: $self->id()');
        (
            exists( $_[0]{"id"} ) ? $_[0]{"id"} : (
                $_[0]{"id"} = do {
                    my $default_value = $_[0]->_build_id;
                    (
                        do {
    ...
    sub requirements {
        @_ == 1 or croak('Reader "requirements" usage: $self->requirements()');
        (
            exists( $_[0]{"requirements"} ) ? $_[0]{"requirements"} : (
                $_[0]{"requirements"} = do {
                    my $default_value = [];
                    ( ref($default_value) eq 'ARRAY' )
                      or croak( "Type check failed in default: %s should be %s",
                        "requirements", "ArrayRef" );
                    $default_value;
                }
            )
        );
    ...
    *all_requirements = sub {
        @_ == 1
          or croak(
"Wrong number of parameters in signature for all_requirements; usage: "
    ...
    sub steps {
        @_ == 1 or croak('Reader "steps" usage: $self->steps()');
        (
            exists( $_[0]{"steps"} ) ? $_[0]{"steps"} : (
                $_[0]{"steps"} = do {
                    my $default_value = [];
                    do {
...
"Type check failed in delegated method: expected %s, got value %s",
    ...
    *all_steps = sub {
        @_ == 1
          or croak(
                "Wrong number of parameters in signature for all_steps; usage: "
              . "\$instance->all_steps()" );
