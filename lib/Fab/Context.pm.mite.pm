{
    ...
    sub new {
        ...
        # param declaration, file lib/Fab/Context.pm, line 11
        do {
            my $value =
              exists( $args->{"log_level"} ) ? $args->{"log_level"} : "0";
            (
                do {
              ...
              "log_level", "IntRange[0,5]";
            $self->{"log_level"} = $value;
        };

        # Attribute blueprint (type: Object)
        # param declaration, file lib/Fab/Context.pm, line 16
        croak "Missing key in constructor: blueprint"
          unless exists $args->{"blueprint"};
        blessed( $args->{"blueprint"} )
          or croak "Type check failed in constructor: %s should be %s",
          "blueprint", "Object";
        ...
        my @unknown = grep not(/\A(?:blueprint|log_level)\z/), keys %{$args};
        @unknown
          and croak(
    ...
    # field declaration, file lib/Fab/Context.pm, line 37
    sub _already_fabricated {
        @_ == 1
          or croak(
            'Reader "_already_fabricated" usage: $self->_already_fabricated()');
        (
            exists( $_[0]{"_already_fabricated"} )
            ? $_[0]{"_already_fabricated"}
            : (
                $_[0]{"_already_fabricated"} = do {
                    my $default_value = {};
                    ( ref($default_value) eq 'HASH' )
                      or croak( "Type check failed in default: %s should be %s",
                        "_already_fabricated", "HashRef" );
                    $default_value;
                }
            )
        );
    ...
    *get_already_fabricated = sub {
        @_ >= 2
          or croak(
"Wrong number of parameters in signature for get_already_fabricated; usage: "
              . "\$instance->get_already_fabricated(\$key)" );
        my $shv_self         = shift;
        my $shv_ref_invocant = do { $shv_self->_already_fabricated };
        scalar(@_) > 1
          ? @{$shv_ref_invocant}{@_}
          : ($shv_ref_invocant)->{ $_[0] };
    };
    *set_already_fabricated = sub {
        @_ >= 3
          or croak(
"Wrong number of parameters in signature for set_already_fabricated; usage: "
              . "\$instance->set_already_fabricated(\$key, \$value, ...)" );
        my $shv_self         = shift;
        my $shv_ref_invocant = do { $shv_self->_already_fabricated };
        my (@shv_params)     = @_;
                ...
                do {
                    ...
                    defined( $shv_params[$shv_tmp] ) and do {
                        ref( \$shv_params[$shv_tmp] ) eq 'SCALAR'
                          or ref( \( my $val = $shv_params[$shv_tmp] ) ) eq
                          'SCALAR';
                    }
                  }
                  or croak(
"Type check failed in delegated method: expected %s, got value %s",
        ...
        @{$shv_ref_invocant}{ @shv_params[@shv_keys_idx] } =
          @shv_params[@shv_values_idx];
        wantarray
          ? @{$shv_ref_invocant}{ @shv_params[@shv_keys_idx] }
          : ($shv_ref_invocant)->{ $shv_params[ $shv_keys_idx[0] ] };
    };
    ...
    if ($__XS) {
        ...
        *blueprint = sub {
            @_ == 1 or croak('Reader "blueprint" usage: $self->blueprint()');
            $_[0]{"blueprint"};
        };
    }
    ...
    if ($__XS) {
        ...
        *log_level = sub {
            @_ == 1 or croak('Reader "log_level" usage: $self->log_level()');
            $_[0]{"log_level"};
        };
    }

    # Accessors for settings
    # field declaration, file lib/Fab/Context.pm, line 26
    sub fresh_settings {
        defined wantarray
    ...
    sub settings {
        @_ == 1 or croak('Reader "settings" usage: $self->settings()');
        (
            exists( $_[0]{"settings"} ) ? $_[0]{"settings"} : (
                $_[0]{"settings"} = do {
                    my $default_value = {};
                    ( ref($default_value) eq 'HASH' )
                      or croak( "Type check failed in default: %s should be %s",
    ...
    *get_setting = sub {
        @_ >= 2
          or croak(
            "Wrong number of parameters in signature for get_setting; usage: "
    ...
    *set_setting = sub {
        @_ >= 3
          or croak(
            "Wrong number of parameters in signature for set_setting; usage: "
...
"Type check failed in delegated method: expected %s, got value %s",
    ...
    # Accessors for stack
    # field declaration, file lib/Fab/Context.pm, line 47
    sub stack {
        @_ == 1 or croak('Reader "stack" usage: $self->stack()');
        (
            exists( $_[0]{"stack"} ) ? $_[0]{"stack"} : (
                $_[0]{"stack"} = do {
                    my $default_value = [];
    ...
    *_pop_stack = sub {
        @_ == 1
          or croak(
            "Wrong number of parameters in signature for _pop_stack; usage: "
              . "\$instance->_pop_stack()" );
        1;
    ...
    *empty_stack = sub {
        @_ == 1
          or croak(
            "Wrong number of parameters in signature for empty_stack; usage: "
              . "\$instance->empty_stack()" );
        my $shv_ref_invocant = do { $_[0]->stack };
    ...
    *get_stack = sub {
        @_ == 1
          or croak(
                "Wrong number of parameters in signature for get_stack; usage: "
              . "\$instance->get_stack()" );
        my $shv_ref_invocant = do { $_[0]->stack };
        @{$shv_ref_invocant};
    };
    ...
    # Accessors for stash
    # field declaration, file lib/Fab/Context.pm, line 21
    sub stash {
        @_ == 1 or croak('Reader "stash" usage: $self->stash()');
        (
            exists( $_[0]{"stash"} ) ? $_[0]{"stash"} : (
                $_[0]{"stash"} = do {
                    my $default_value = {};
                    ( ref($default_value) eq 'HASH' )
                      or croak( "Type check failed in default: %s should be %s",
