{
    ...
    sub new {
     ...
     # param declaration, file lib/Fab/Tools/Make2Fab.pm, line 11
        croak "Missing key in constructor: input"
        ...
        do {
            ...
            (
                do {
                    ...
                    package Fab::Mite;
                    ref( $args->{"input"} ) eq 'SCALAR'
                      or ref( $args->{"input"} ) eq 'REF';
                  }
                  or (
                    do {
                        use Scalar::Util ();
                        Scalar::Util::blessed( $args->{"input"} )
                          and $args->{"input"}->isa(q[Path::Tiny]);
                    }
                  )
                  or (
                    do {
                        ...
                        use Scalar::Util ();
                        ( ref( $args->{"input"} )
                              && Scalar::Util::openhandle( $args->{"input"} ) )
                          or ( Scalar::Util::blessed( $args->{"input"} )
                            && $args->{"input"}->isa("IO::Handle") );
                    }
                  )
                  or do {

                    package Fab::Mite;
                    defined( $args->{"input"} ) and do {
                        ref( \$args->{"input"} ) eq 'SCALAR'
                          or ref( \( my $val = $args->{"input"} ) ) eq 'SCALAR';
                    }
                }
            );
          }
          or croak "Type check failed in constructor: %s should be %s",
          "input", "ScalarRef|InstanceOf[\"Path::Tiny\"]|FileHandle|Str";
        ...
        my @unknown = grep not(/\Ainput\z/), keys %{$args};
        @unknown
          and croak(
    ...
    if ($__XS) {
        ...
        *input = sub {
            @_ == 1 or croak('Reader "input" usage: $self->input()');
            $_[0]{"input"};
        };
    }
    ...
    sub output {
        @_ == 1 or croak('Reader "output" usage: $self->output()');
        (
            exists( $_[0]{"output"} ) ? $_[0]{"output"} : (
                $_[0]{"output"} = do {
                    my $default_value = $_[0]->_build_output;
                    do {
    ...
    sub parsed {
        @_ == 1 or croak('Reader "parsed" usage: $self->parsed()');
        (
            exists( $_[0]{"parsed"} ) ? $_[0]{"parsed"} : (
                $_[0]{"parsed"} = do {
                    my $default_value = $_[0]->_build_parsed;
