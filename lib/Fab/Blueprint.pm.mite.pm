{
    ...
    sub new {
        ...
        my @unknown = grep not(/\Adefinition_context\z/), keys %{$args};
        @unknown
          and croak(
    ...
    if ($__XS) {
        ...
        *context = sub {
            @_ == 1 or croak('Reader "context" usage: $self->context()');
            $_[0]{"context"};
        };
    }

    sub _assert_blessed_context {
        my $object = do { $_[0]{"context"} };
        blessed($object) or croak("context is not a blessed object");
        $object;
    }
        ...
        @_ > 1
          ? do {
            do {
                ...
                package Fab::Mite;
                ( ( ref( $_[1] ) eq 'HASH' ) or ( !defined( $_[1] ) ) );
              }
              or croak( "Type check failed in %s: value should be %s",
                "accessor", "HashRef|Undef" );
    ...
    if ($__XS) {
        ...
        *finalized = sub {
            @_ == 1 or croak('Reader "finalized" usage: $self->finalized()');
            $_[0]{"finalized"};
        };
    }
    ...
    *finalize = sub {
        @_ == 1
          or
          croak("Wrong number of parameters in signature for finalize; usage: "
              . "\$instance->finalize()" );
        do {
            my $shv_real_invocant = $_[0]->finalized;
            (
                $_[0]->{"finalized"} = do {
                    ...
                    do {
                        (
                            !ref $shv_final_unchecked
                              and (!defined $shv_final_unchecked
                                or $shv_final_unchecked eq q()
                                or $shv_final_unchecked eq '0'
                                or $shv_final_unchecked eq '1' )
                          )
                          or croak(
"Type check failed in delegated method: expected %s, got value %s",
                            "Bool", $shv_final_unchecked
                          );
                        $shv_final_unchecked;
                    };
                }
    ...
    sub clear_task_lookup {
        @_ == 1
          or croak(
            'Clearer "clear_task_lookup" usage: $self->clear_task_lookup()');
        delete $_[0]{"task_lookup"};
        $_[0];
    }

    sub task_lookup {
        @_ == 1 or croak('Reader "task_lookup" usage: $self->task_lookup()');
        (
            exists( $_[0]{"task_lookup"} ) ? $_[0]{"task_lookup"} : (
                $_[0]{"task_lookup"} = do {
                    my $default_value = $_[0]->_build_task_lookup;
    ...
    sub tasks {
        @_ == 1 or croak('Reader "tasks" usage: $self->tasks()');
        (
            exists( $_[0]{"tasks"} ) ? $_[0]{"tasks"} : (
                $_[0]{"tasks"} = do {
                    my $default_value = [];
                    do {
        ...
        for my $shv_value (@_) {
            do {
                (
                    do {
                        use Scalar::Util ();
                        Scalar::Util::blessed($shv_value)
                          and $shv_value->isa(q[Fab::Task]);
                    }
                  )
                  or croak(
"Type check failed in delegated method: expected %s, got value %s",
                    "InstanceOf[\"Fab::Task\"]", $shv_value
                  );
                $shv_value;
            };
        }
    ...
    *all_tasks = sub {
        @_ == 1
          or croak(
                "Wrong number of parameters in signature for all_tasks; usage: "
              . "\$instance->all_tasks()" );
