{
    ...
    sub new {
        ...
        do {
            ...
            package Fab::Mite;
            defined( $args->{"key"} ) and do {
                ref( \$args->{"key"} ) eq 'SCALAR'
                  or ref( \( my $val = $args->{"key"} ) ) eq 'SCALAR';
            }
          }
          or croak "Type check failed in constructor: %s should be %s", "key",
          "Str";
        $self->{"key"} = $args->{"key"};

        # Attribute value (type: Any)
        # param declaration, file lib/Fab/Step/Set.pm, line 16
        croak "Missing key in constructor: value"
        ...
        my @unknown = grep not(/\A(?:definition_context|key|task|value)\z/),
          keys %{$args};
        @unknown
          and croak(
    ...
    if ($__XS) {
        ...
        *key = sub {
            @_ == 1 or croak('Reader "key" usage: $self->key()');
            $_[0]{"key"};
        };
    }
    ...
    if ($__XS) {
        ...
        *value = sub {
            @_ == 1 or croak('Reader "value" usage: $self->value()');
            $_[0]{"value"};
        };
    }
