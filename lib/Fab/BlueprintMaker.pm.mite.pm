{
    ...
    sub _current_task {
        @_ > 1
          ? do {
            blessed( $_[1] ) && $_[1]->isa("Fab::Task")
              or croak( "Type check failed in %s: value should be %s",
                "accessor", "InstanceOf[\"Fab::Task\"]" );
            $_[0]{"_current_task"} = $_[1];
            $_[0];
          }
          : ( $_[0]{"_current_task"} );
    }

    sub _clear__current_task {
        @_ == 1
          or croak(
'Clearer "_clear__current_task" usage: $self->_clear__current_task()'
          );
        delete $_[0]{"_current_task"};
        $_[0];
    }
    ...
    sub blueprint {
        ...
        (
            exists( $_[0]{"blueprint"} ) ? $_[0]{"blueprint"} : (
                $_[0]{"blueprint"} = do {
                    my $default_value = $_[0]->_build_blueprint;
                    blessed($default_value)
                      && $default_value->isa("Fab::Blueprint")
                      or croak(
                        "Type check failed in default: %s should be %s",
                        "blueprint",
                        "InstanceOf[\"Fab::Blueprint\"]"
                      );
                    $default_value;
                }
            )
